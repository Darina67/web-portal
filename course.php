  ...
  <div class="hamburger-menu">
    ...
    <section class="section course-content">
     <div class="container">
      ...
      <?php
      $qr_coursedata = $connect->prepare("SELECT * FROM `courses` where `id` = $course_id");
      $qr_coursedata->execute();
      ?>  
      <?php while($row = $qr_coursedata->fetch()) {  ?>  
        <div class="course">
          <div class="course__title"><?=$row['TITLE']?></div>
          <div class="course__avatar">
            <img src="assets/img/course.jpg" alt="Курс">
          </div>
          <div class="course__info">
                <?=$row['INFORMATION']?>
                <br> 
                <a class="course__info-test" href="tests.php">Пройти тест</a>
          </div>
      </div>
      <?php } ?>
    </div>
